...
class PolicyAnalysis:
    ...
    def _handle_response(self, response, resource_name, policy_name, operation_name):
        ...
        if isinstance(response, list):
            findings = [
                self._build_policy_analysis_finding(r, operation_name)
                for r in response
    ...
    def run(self, plan):
        ...
        for ref, policy_str in policies.items():
            ...
            cache = (
                ...
                else self.resource_policy_cache
            )
            ...
            self._handle_response(
                response, resource_name, policy_name, self.operation_name
            )
...
class Comparator(PolicyAnalysis):
    ...
    def _call_api(self, policy, policy_type):
        if (
            self.reference_policy_type == "identity"
            and policy_type != "IDENTITY_POLICY"
        ):
            return []
        if (
            self.reference_policy_type == "resource"
            and policy_type != "RESOURCE_POLICY"
        ):
            return []
        try:
...
class AccessChecker(PolicyAnalysis):
        ...
        if resources:
            if len(resources) > RESOURCES_MAX_ITEMS:
                raise ApplicationError(
                    "Too many resource ARNs were specified. You may only specify up to 100 resource ARNs."
                )
    ...
    def _call_api(self, policy, policy_type):
        responses = []
        failed_response = {"result": "FAIL", "reasons": []}
        for access in self.accesses:
            LOGGER.info(f"Batching actions {access}")
            try:
                response = self.client.check_access_not_granted(
                    policyType=policy_type, policyDocument=policy, access=access
                )
            except ClientError as error:
            ...
            if response.get("result") == "FAIL":
                ...
                if reasons is not None:
                    ...
                    failed_response["reasons"].append({"accessInput": access})
...
class PublicAccessChecker(PolicyAnalysis):
    ...
    def _call_api(self, policy, resource_type):
        try:
            response = self.client.check_no_public_access(
                policyDocument=policy, resourceType=resource_type
            )
        except ClientError as error:
            return error.response
    ...
    def run(self, plan):
        ...
        for ref, policy_str in policies.items():
            ...
            aws_resource_type = RESOURCE_TYPE_MAP.get(
                policy_resource_type, RESOURCE_TYPE_MAP.get(policy_attribute_type)
            )
            # Skip resource type if it is not one of the officially supported types
            if not aws_resource_type:
                LOGGER.info(
                    f"Resource type {policy_resource_type} not found in the mapping of terraform to official AWS types"
                )
                continue
            policy_name = ".".join(ref.split(".")[0:-1])
            resource_name = plan.getResourceName(policy_name)
