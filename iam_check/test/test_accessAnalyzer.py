...
class TestAccessAnalyzer:
    ...
    def test_a2_policy_check_no_new_access_mixed_policies(self):
        for dir_name in ["multiple_policies", "role_inline_assume_policy"]:
            ...
            for policy_type in ["resource", "identity"]:
                reference_policy = _load_json_file(
                    f"test/{dir_name}/{policy_type}_reference_policy.json"
                )
                check = Comparator(
                    "us-west-2",
                    json.dumps(reference_policy),
                    reference_policy_type=f"{policy_type}",
                )
                check.run(plan)
                test_findings = _load_json_file(
                    f"test/{dir_name}/{policy_type}_findings.json"
                )
                assert (
                    Reporter(None, ["ERROR", "SECURITY_WARNING"], None)
                    .build_report_from(check.findings)
                    .to_json()
                    == test_findings
                )
    ...
    def test_a2_policy_check_no_public_access(self):
        ...
        check.run(plan)
        test_findings = _load_json_file(
            f"test/public_access/public_access_findings.json"
        )
        assert (
            Reporter(None, ["ERROR", "SECURITY_WARNING"], None)
            .build_report_from(check.findings)
    ...
    def test_a2_check_no_access_granted_actions_and_resources(self):
        ...
        check.run(plan)
        findings = _load_json_file(
            "test/no_access_granted/resources_and_actions_findings.json"
        )
        assert (
            Reporter(None, ["ERROR", "SECURITY_WARNING"], None)
            .build_report_from(check.findings)
